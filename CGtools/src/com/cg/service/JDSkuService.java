...
public class JDSkuService {
	...
	public boolean addJDSku(JDSku jdSku){
		...
		try {
			...
			ps.setString(7, PriceTool.roundToString(jdSku.getPublishingPrice()));
	...
	public Map<String,Object> queryALL(String skuId,String cgId,String name,Boolean important,Boolean jd_instock,String type,String special_type,String barcode,int page,String sortName, String sort){
		List<JDSku> allSkus = queryJDSkus(skuId, cgId,name,important,jd_instock,type,special_type,barcode,sortName,sort);
		int allRows = allSkus.size();
		int allPages = allRows%ITEMS_IN_PAGE==0?allRows/ITEMS_IN_PAGE:allRows/ITEMS_IN_PAGE+1;
		if(page>allPages){
			return null;
		}
		List<JDSku> querySkus = allSkus.subList((page-1)*ITEMS_IN_PAGE, (page)*ITEMS_IN_PAGE>allRows?allRows:(page)*ITEMS_IN_PAGE);
	...
	public Map<String,Object> queryALL(int page,String sortName, String sort){
		return queryALL(null,null,null,null,null,null,null,null,page,sortName,sort);
	}
	
	public List<JDSku> queryALLItems(){
		List<JDSku> allSkus = queryJDSkus(null,null,null,null,null,null,null,null);
		return allSkus;
	}
	...
	public List<JDSku> queryJDSkus(String skuId,String cgId,String name,Boolean important,String special_type,String barcode,Boolean jd_instock,String type){
		return queryJDSkus(skuId,cgId,name,important,jd_instock,type, special_type,barcode,null,null);
	}
	...
	public List<JDSku> queryJDSkus(String skuId,String cgId,String name,Boolean important,Boolean jd_instock,String type,String special_type, String barcode, String sortName,String sort){
		...
		try {
			...
			if(!StringUtils.isBlank(barcode)){
				ps.setString(getIndex(skuId,cgId,name,barcode), "%"+barcode+"%");
			}
			if(important!=null){
				ps.setBoolean(getIndex(skuId,cgId,name,barcode,important), important);
			}
			if(!StringUtils.isBlank(type)){
				ps.setString(getIndex(skuId,cgId,name,barcode,important,type), type);
			}
			if(!StringUtils.isBlank(special_type)){
				ps.setString(getIndex(skuId,cgId,name,barcode,important,type,special_type), special_type);
			}
			if(jd_instock!=null){
				ps.setBoolean(getIndex(skuId,cgId,name,barcode,important,type,special_type,jd_instock), jd_instock);
			}
			rs = ps.executeQuery();
			while(rs.next()){
	...
	public List<JDSku> queryAllJDWithPriceSkus(String[] itemTypes, String special_type, String[] exclude_itemTypes, String exclude_special_type){
		StringBuilder sql = new StringBuilder(); 
		if((exclude_itemTypes == null || exclude_itemTypes.length==0) && StringUtils.isBlank(exclude_special_type)){
			...
			sql.append(" and sku_id not in ( select sku_id from sku b where b.jd_price is not null");
			if(exclude_itemTypes!=null && exclude_itemTypes.length>0){
				sql.append(" and type in (" + getInSql(exclude_itemTypes)+")");
			}
	...
	public boolean updatePriceAndStock(String skuId, String price, Boolean stock, String updateDateStr){
		String sql = "";
		if(stock == null){
	...
	public List<String> updateSkuPrice(String idsStr){
		...
		sb.append(")");
		List<JDSku> jdskus = queryJDSkuBySQL(sb.toString(), Collections.emptyList());
		return updateSkuPrice(jdskus);
	}
	
	public List<String> updateSkuPrice(boolean searchAllItems, String[] itemTypes){
	...
	public List<String> updateSkuPromotion(boolean searchAllItems, String[] itemTypes){
		List<JDSku> jdskus = null;
	...
	public List<String> updateSkuPromotions(List<JDSku> jdskus){
		...
		for(JDSku jdSku : jdskus){
			try{
				RemoteInfoServiceIF remoteInfoService = RemoteInfoServiceHelper.getRemoteInfoServiceIF(jdSku);
				String promotions = remoteInfoService.getPromotionInfo(jdSku.getSku_id());
				System.out.println(promotions);
				DBUtil.executeUpdate(sql, new Object[]{promotions,jdSku.getSku_id()});
			}catch (Exception e) {
				errorMessage.add("sku:" + jdSku.getSku_id() + " has some error");
	...
	public List<String> updateSkuPrice(List<JDSku> jdskus){
		...
		for(JDSku jdSku : jdskus){
			if("10".equals(jdSku.getTypeId())){
				...
				try {
					String price = RemoteInfoServiceHelper.getInstance().getPrice(httpclient, jdSku);
					if(StringUtils.isBlank(price)){
						errorMessage.add("Price is null| Sku:" + generateLink(jdSku.getSku_id()) + " error in get Price & Stock");
						continue;
					}
					Boolean stock = RemoteInfoServiceHelper.getInstance().hasStock(httpclient, jdSku);
					if(stock == null){
						errorMessage.add("Sku:" + generateLink(jdSku.getSku_id()) + " error in get Stock");
					}
					updatePriceAndStock(jdSku.getSku_id(),price,stock,updateDateStr);
					System.out.println("sku:" + jdSku.getSku_id() + " price:" + price +" stock:" + stock);
				} catch (CGexception e) {
					errorMessage.add("Sku:" + generateLink(jdSku.getSku_id()) + " error in get Price & Stock");
					// TODO Auto-generated catch block
					e.printStackTrace();
				}  catch (Exception e) {
					errorMessage.add("Exception Sku:" + generateLink(jdSku.getSku_id()) + " error in get Price & Stock");
					e.printStackTrace();
				} finally{
					try {
	...
	private static String generateLink(String skuId){
		return "<a target=\"_blank\" href=\"http://click.union.jd.com/JdClick/?unionId=11406&t=4&to=http://www.jd.com/product/"+skuId+".html\">"+skuId+"</a>";
	}
}
